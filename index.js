...
const stickItems = [
 ...
 const gloveItems = [ { type: "Glove", brand: "Bauer", style: "Vapor (2022)", size: "Junior", price: "$169.99", imageUrl: "https://cdn.shopify.com/s/files/1/0505/8838/5453/products/b1af3b490876395f8e8d17ea4a7efd4e_500x500_crop_center.jpg?v=1657318715", linkRef: "https://www.sourceforsports.ca/products/bauer-vapor-3x-pro-junior-hockey-gloves-2022" },
  ...
  const helmetItems = [{ type: "Helmet,", brand: "Warrior,", style: "Covert CF100,", size: "Senior", price: "199.99", imageUrl: "https://cdn.shopify.com/s/files/1/0505/8838/5453/products/d2fabfa31a21f6f456e43eae7ba206d5_500x500_crop_center.jpg?v=1656671578", linkRef: "https://www.sourceforsports.ca/products/warrior-covert-cf-100-hockey-helmet" },
...
const itemsGroupByContainers = putItemsIntoContainers(stickItems, 8)
...
const putGloveItemsIntoContainers = (gloveItems, cutOffnumber) => {
    let cutOff = cutOffnumber - 1
    let container = []
    const results = []
    gloveItems.map((item, index) => {

        if (index <= cutOff) {
            container.push(item)
        }
        if (index === cutOff || index == gloveItems.length - 1) {
            results.push(container)
            cutOff += cutOffnumber
            container = [];
        }
...
const putHelmetItemsIntoContainers=(helmetItems,cutOffnumber) =>{
    let cutOff = cutOffnumber -1
    let container =[]
    const results=[]
    helmetItems.map((item,index) => {
        ...
        if(index === cutoff || helmetItems.length -1){
            results.push(container)
            cutoff += cutOffnumber
            container=[];
        }
