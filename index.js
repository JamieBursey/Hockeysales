...
const parentArray = document.getElementById("item-container")
const subArray=document.getElementById("item-container")
itemsGroupByContainers.forEach((container) => {
    items.forEach((item) => {
        ...
        parentArray.appendChild(element);
...
items.forEach((item)=> {  
     ...
     const img=document.createElement("img")
     element.textContent=items.type + item.brand;
     element.appendChild(img);
     const allImages=document.querySelector("img");
     img.setAttribute("src",item.imageUrl);
     img.setAttribute("width", "100");
     img.setAttribute("height", "100");
     subArray.appendChild(element);
})
})
